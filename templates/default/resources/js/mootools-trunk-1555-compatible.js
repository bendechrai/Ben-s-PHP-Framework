...
new Document(document);window.extend=document.extend=function(A){for(var B in A){this[B]=A[B];}};window[Browser.Engine.name]=window[Browser.Engine.name+Browser.Engine.version]=true;
window.ie=window.trident;window.ie6=window.trident4;window.ie7=window.trident5;Array.implement({every:function(C,D){for(var B=0,A=this.length;B<A;B++){if(!C.call(D,this[B],B,this)){return false;
}}return true;},filter:function(D,E){var C=[];for(var B=0,A=this.length;B<A;B++){if(D.call(E,this[B],B,this)){C.push(this[B]);}}return C;},clean:function(){return this.filter($defined);
...
B.push((C.length==1)?"0"+C:C);}return(D)?B:"#"+B.join("");}});Array.implement({copy:function(B,A){return $A(this,B,A);}});Array.alias({erase:"remove",combine:"merge"});
...
Hash.alias({getKeys:"keys",getValues:"values",has:"hasKey",combine:"merge"});var Abstract=Hash;var Event=new Native({name:"Event",initialize:function(A,F){F=F||window;
...
}if(!(function(){while(L&&L.nodeType==3){L=L.parentNode;}return true;}).create({attempt:Browser.Engine.gecko})()){L=false;}}}}return $extend(this,{event:A,type:J,page:I,client:C,rightClick:E,wheel:H,relatedTarget:L,target:G,code:B,key:M,shift:A.shiftKey,control:A.ctrlKey,alt:A.altKey,meta:A.metaKey});
...
};self.parentOf=function(descendant){return descendant._parent_.apply(this,Array.slice(arguments,1));};}};Class.empty=$empty;Class.prototype.extend=function(A){A.Extends=this;
...
},clearChain:function(){if(this.$chain){this.$chain.empty();}return this;}});var Events=new Class({addEvent:function(C,B,A){C=Events.removeOn(C);if(B!=$empty){this.$events=this.$events||{};
...
},this);return this;},removeEvent:function(B,A){B=Events.removeOn(B);if(!this.$events||!this.$events[B]){return this;}if(!A.internal){this.$events[B].erase(A);
}return this;},removeEvents:function(C){for(var D in this.$events){if(C&&C!=D){continue;}var B=this.$events[D];for(var A=B.length;A--;A){this.removeEvent(D,B[A]);
...
if(Browser.Engine.trident){CollectGarbage();}});Element.extend=Element.implement;Elements.extend=Elements.implement;Element.implement({getFormElements:function(){return this.getElements("input, textarea, select");
},replaceWith:function(A){A=$(A);this.parentNode.replaceChild(A,this);return A;},removeElements:function(){return this.dispose();}});Element.alias({dispose:"remove",getLast:"getLastChild"});
...
},getHTML:function(){return this.get("html");},getTag:function(){return this.get("tag");}});Element.Properties.events={set:function(A){this.addEvents(A);
}};Native.implement([Element,Window,Document],{addEvent:function(E,G){var H=this.retrieve("events",{});H[E]=H[E]||{keys:[],values:[]};if(H[E].keys.contains(G)){return this;
}H[E].keys.push(G);var F=E,A=Element.Events.get(E),C=G,I=this;if(A){if(A.onAdd){A.onAdd.call(this,G);}if(A.condition){C=function(J){if(A.condition.call(this,J)){return G.call(this,J);
}return false;};}F=A.base||F;}var D=function(){return G.call(I);};var B=Element.NativeEvents[F]||0;if(B){if(B==2){D=function(J){J=new Event(J,I.getWindow());
if(C.call(I,J)===false){J.stop();}};}this.addListener(F,D);}H[E].values.push(D);return this;},removeEvent:function(D,C){var B=this.retrieve("events");if(!B||!B[D]){return this;
}var G=B[D].keys.indexOf(C);if(G==-1){return this;}var A=B[D].keys.splice(G,1)[0];var F=B[D].values.splice(G,1)[0];var E=Element.Events.get(D);if(E){if(E.onRemove){E.onRemove.call(this,C);
}D=E.base||D;}return(Element.NativeEvents[D])?this.removeListener(D,F):this;},addEvents:function(A){for(var B in A){this.addEvent(B,A[B]);}return this;
},removeEvents:function(B){var A=this.retrieve("events");if(!A){return this;}if(!B){for(var C in A){this.removeEvents(C);}A=null;}else{if(A[B]){while(A[B].keys[0]){this.removeEvent(B,A[B].keys[0]);
...
},getOffsets:function(){var I=this,H={x:0,y:0};if(B(this)){return H;}while(I&&!B(I)){H.x+=I.offsetLeft;H.y+=I.offsetTop;if(Browser.Engine.gecko){if(!F(I)){H.x+=C(I);
...
}});Elements.implement({filterByTag:function(A){return this.filter(A);},filterByClass:function(A){return this.filter("."+A);},filterById:function(A){return this.filter("#"+A);
},filterByAttribute:function(B,A,C){return this.filter("["+B+(A||"")+(C||"")+"]");}});var $E=function(A,B){return($(B)||document).getElement(A);};var $ES=function(A,B){return($(B)||document).getElements(A);
...
};Fx.Durations={"short":250,normal:500,"long":1000};Fx.implement({custom:function(B,A){return this.start(B,A);},clearTimer:function(){return this.cancel();
},stop:function(){return this.cancel();}});Fx.Base=Fx;Fx.CSS=new Class({Extends:Fx,prepare:function(D,E,B){B=$splat(B);var C=B[1];if(!$chk(C)){B[1]=B[0];
...
}});Fx.Morph=new Class({Extends:Fx.CSS,initialize:function(B,A){this.element=this.subject=$(B);this.parent(A);},set:function(A){if(typeof A=="string"){A=this.search(A);
...
}return this.retrieve("morph");}};Element.implement({morph:function(A){this.get("morph").start(A);return this;}});Fx.Styles=Fx.Morph;Element.implement({effects:function(A){return new Fx.Morph(this,A);
...
var XHR=new Class({Extends:Request,options:{update:false},initialize:function(B,A){this.parent(A);this.url=B;},request:function(A){return this.send(this.url,A||this.options.data);
},send:function(A,B){if(!this.check(arguments.callee,A,B)){return this;}return this.parent({url:A,data:B});},success:function(B,A){B=this.processScripts(B);
if(this.options.update){$(this.options.update).empty().set("html",B);}this.onSuccess(B,A);},failure:function(){this.fireEvent("failure",this.xhr);}});var Ajax=XHR;
Request.HTML=new Class({Extends:Request,options:{update:false,evalScripts:true,filter:false},processHTML:function(C){var B=C.match(/<body[^>]*>([\s\S]*?)<\/body>/i);
...
},success:function(A){this.response.json=JSON.decode(A,this.options.secure);this.onSuccess(this.response.json,A);}});JSON.Remote=new Class({options:{key:"json"},Extends:Request.JSON,initialize:function(B,A){this.parent(A);
this.onComplete=$empty;this.url=B;},send:function(A){if(!this.check(arguments.callee,A)){return this;}return this.parent({url:this.url,data:{json:Json.encode(A)}});
},failure:function(){this.fireEvent("failure",this.xhr);}});Fx.Slide=new Class({Extends:Fx,options:{mode:"vertical"},initialize:function(B,A){this.addEvent("complete",function(){this.open=(this.wrapper["offset"+this.layout.capitalize()]!=0);
